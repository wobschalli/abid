...
class Scheduler
  ...
  def collect_scheduled_message(event)
    ...
    drivers = []
    if driver_emoji
      target_d_reaction = message.reactions.find { |r| r.to_s == driver_emoji.to_reaction.to_s }
      d_users = target_d_reaction ? target_d_reaction.users : []
    ...
    riders = []
    if rider_emoji
      target_r_reaction = message.reactions.find { |r| r.to_s == rider_emoji.to_reaction.to_s }
      r_users = target_r_reaction ? target_r_reaction.users : []
